...
export default function Section({ title, closing = "none", children, ...rest }: PropsWithChildren<Props>) {
  const CLOSING = {
    curlyBrace: [<>&nbsp;=&nbsp;&#123;</>, <>&#125;;</>],
    squareBrace: [<>&nbsp;=&nbsp;&#91;</>, <>&#93;;</>],
    parenthesis: [<>&nbsp;=&nbsp;&#40;</>, <>&#41;;</>],
  ...
  return (
    <div className="max-w-7xl mx-auto w-full pt-12">
      ...
      <p className="text-right text-3xl sm:text-4xl text-primary-400 dark:text-primary-400">{CLOSING[closing][1]}</p>
    </div>
  );
}
